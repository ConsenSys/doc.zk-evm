...
broadcasts it to Linea. From here, transactions head straight to the **mempool**, similar to 
...
the transaction's data—as contained in its trace—to generate proof.
...
- Blob data, which contains L2 transaction and messaging data. Once shared with L1, blob data
